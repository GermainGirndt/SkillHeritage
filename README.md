...
```mermaid
graph TD
    User((Mechanic)) -->|Records Video| App[Android App - Expo]
    App -->|Uploads MP4| Server[Backend - Uberspace]
    Server -->|Stores Data| DB[(MongoDB - GridFS)]
    Server -->|Sends Audio| AI{OpenAI API}
    AI -->|Transcript & Steps| Server
    Server -->|Dynamic Tutorial| App
    App -->|Playback| Novice((New Worker))
