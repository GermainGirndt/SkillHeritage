...
export default function TutorialDetailScreen() {
  ...
  useEffect(() => {
    ...
    async function loadTutorial() {
      if (!id) return;
      try {
        // Fetching real data using the integrated API Client
        const data = await TutorialApiClient.getById(id as string);
        setTutorial(data);

        // Polling: If AI processing is not yet finished, refresh data every 5 seconds
        if (data.processingStatus !== "completed") {
          timer = setTimeout(loadTutorial, 5000);
        }
      } catch (e) {
        console.error("Failed to load tutorial from server:", e);
      } finally {
        setLoading(false);
      }
    }
      ...
      <ScrollView style={styles.content}>
        {tutorial.processingStatus !== "completed" ? (
          ...
          <View style={styles.manualContainer}>
            <View style={styles.aiBadge}><Text style={styles.aiBadgeText}>GENERATED BY AI</Text></View>
